	...
	: Layer("ExampleLayer"), m_CameraController(1280.0f / 720.0f)
{
	...
	Asaurus::Ref<Asaurus::IndexBuffer> indexBuffer = Asaurus::IndexBuffer::Create(indices, sizeof(indices) / sizeof(uint32_t));
	...
	Asaurus::Ref<Asaurus::IndexBuffer> squareIB = Asaurus::IndexBuffer::Create(squareIndices, sizeof(squareIndices) / sizeof(uint32_t));
	...
	m_Shader = Asaurus::Shader::Create("VertexPosColor", vertexSrc, fragmentSrc);
...
void ExampleLayer::OnUpdate(Asaurus::Timestep ts)
{
	// Update
...
void ExampleLayer::OnEvent(Asaurus::Event& e)
{
